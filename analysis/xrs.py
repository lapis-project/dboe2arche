...
for x in files:
    ...
    for y in doc.any_xpath(".//tei:entry[.//tei:xr]"):
        try:
            xml_id = get_xmlid(y)
        except KeyError:
            continue
        items[xml_id] = []
        for node in any_xpath(y, ".//tei:xr"):
            items[xml_id].append(node_to_json(node))

all_keys = set()

# go through XR items and save their first-level key names
for x in items.values():
    for y in x:
        all_keys.update(y)

with open("xrs.json", "w", encoding="utf-8") as fp:
    json.dump(items, fp, ensure_ascii=False, indent=4)

with open("xrs_attribute_names.json", "w", encoding="utf-8") as fp:

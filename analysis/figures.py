...
items = {}
files = sorted(glob.glob("./data/h41*.xml"))
attribute_names = set()
for x in files:
    doc = TeiReader(x)
    print(f"processing {x}")
    for y in doc.any_xpath(".//tei:entry[./tei:figure]"):
        try:
            xml_id = get_xmlid(y)
        except KeyError:
            continue
        items[xml_id] = []
        for node in any_xpath(y, "./tei:figure"):
...
with open("figures.json", "w", encoding="utf-8") as fp:
    json.dump(items, fp, ensure_ascii=False, indent=4)

with open("figures_attribute_names.json", "w", encoding="utf-8") as fp:

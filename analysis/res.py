...
items = {}
files = sorted(glob.glob("./data/*.xml"))[0:12]
attribute_names = set()
for x in files:
    doc = TeiReader(x)
    print(f"processing {x}")
    for y in doc.any_xpath(".//tei:entry[./tei:cit/tei:re]"):
        try:
            xml_id = get_xmlid(y)
        except KeyError:
            continue
        items[xml_id] = []
        for node in any_xpath(y, "./tei:cit/tei:re"):
...
with open("res_attribute_names.json", "w", encoding="utf-8") as fp:
